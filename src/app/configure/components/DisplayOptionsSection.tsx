...
export function DisplayOptionsSection(props: DisplayOptionsProps) {
    ...
    return (
      <div className="mt-7">
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex items-center justify-between rounded-lg bg-zinc-800/35 px-3.5 py-3">
            <div>
              <p className="text-[13px] font-medium text-white">Hide Films Without a Home Release</p>
              <p className="mt-0.5 text-[11px] text-zinc-500">Hide films not yet available digitally, physically or on TV</p>
            </div>
            <Toggle
              enabled={props.hideNoHomeRelease}
              onToggle={() => props.onHideNoHomeReleaseChange(!props.hideNoHomeRelease)}
            />
          </div>
          <div className="flex items-center justify-between rounded-lg bg-zinc-800/35 px-3.5 py-3">
            <div>
  ...
  const items = [
    ...
    { key: "hideNoHomeRelease" as const, label: "Hide Films Without a Home Release", description: "Hide films not yet available digitally, physically or on TV", defaultOn: false },
